...
use App\DTOs\Report\GeneralReportFilterDTO;
use App\DTOs\Report\EmployeeReportFilterDTO;
use App\Models\User;
...
class ReportService
{
    ...
    public function generateAttendanceMonthlyReport(User $user, AttendanceReportFilterDTO $filters): void
    {
        ...
        foreach ($period as $date) {
            ...
            $dayRecords = $groupedAttendance->get($dateStr, collect());

            foreach ($employees as $employee) {
                // Use Object-style access for $employee
                $empId = $employee->user_id ?? $employee['user_id'];

                if ($dayRecords->has($empId)) {
                    $record = $dayRecords->get($empId);
                    if (!$record->employee) {
                        $record->employee = (object)$employee;
                    }
                    $fullData->push($record);
                } else {
                    // Create Empty Record with Shift Info from cache
                    ...
                    $shiftInfo = $employeeShiftCache[$empId] ?? [];
        ...
        $this->generateAttendancePdf($fullData, $title, $filters->companyId, 'monthly');
    }
    ...
    public function generateAttendanceFirstLastReport(User $user, AttendanceReportFilterDTO $filters): void
    {
        ...
        // Correct signature: userId, companyId
        $rawEmployees = $this->permissionService->getEmployeesByHierarchy($user->user_id, $filters->companyId);

        // Convert to Collection for easier manipulation
        $employees = collect($rawEmployees);

        // Filter by selected Employee IDs if any
        if (!empty($filters->employeeIds)) {
            $employees = $employees->whereIn('user_id', $filters->employeeIds);
        } elseif ($filters->employeeId) {
            $employees = $employees->where('user_id', $filters->employeeId);
        ...
        // 3. Group Attendance by Date -> Employee
        $groupedAttendance = $attendanceData->groupBy('attendance_date')->map(function ($items) {
            return $items->keyBy('employee_id');
        });

        // 4. Build Full Dataset (All Dates x All Employees)
        $fullData = collect();
        if ($startDate && $endDate) {
            ...
            foreach ($period as $date) {
                ...
                foreach ($employees as $employee) {
                    ...
                    if ($dayRecords->has($empId)) {
                        $fullData->push($dayRecords->get($empId));
                    } else {
                        // Empty Record with default shift info
                        $emptyRecord = new \stdClass();
                        ...
                        // Apply default shift from cache
                        $shiftInfo = $defaultShifts->get($empId);
                        if ($shiftInfo) {
                            foreach ((array)$shiftInfo as $key => $value) {
                                $emptyRecord->$key = $value;
                            }
                        }
            ...
            // Fallback if no dates (shouldn't happen with valid filters)
        ...
        $this->generateFirstLastPdf($fullData, $title, $filters->companyId, $dateRange);
    }
    ...
    public function generateAttendanceTimeRecordsReport(User $user, AttendanceReportFilterDTO $filters): void
    {
        ...
        $this->generateTimeRecordsPdf($data, $title, $filters->companyId, $employee, $dateRange);
    }
    ...
    public function generateAttendanceDateRangeReport(User $user, AttendanceReportFilterDTO $filters): void
    {
        ...
        $this->generateDateRangePdf($data, $title, $filters->companyId, $employee, $dateRange);
    }
    ...
    public function generateTimesheetReport(User $user, AttendanceReportFilterDTO $filters): void
    {
        ...
        $this->generateTimesheetPdf($data, $title, $filters->companyId, $dateRange);
    }
    ...
    public function generateLoanReport(User $user, int $companyId, array $filters = []): void
    {
        ...
        $this->generateLoanPdf($data, $title, $companyId, $filters);
    }
    ...
    public function generateAwardsReport(User $user, int $companyId, array $filters = []): void
    {
        ...
        $this->generateAwardsPdf($awards, 'تقرير المكافآت', $companyId, $dateRange, $user);
    }
    ...
    public function generatePromotionsReport(User $user, int $companyId, array $filters = []): void
    {
        ...
        $this->generatePromotionsPdf($data, 'تقرير الترقيات', $companyId, $dateRange, $user);
    }
     ...
     */
    // Completed
    private function generateAttendancePdf(Collection $data, string $title, int $companyId, string $type): void
    {
        // Headers matching the requested screenshot (Right to Left)
        // الاسم | يوم | التاريخ | اسم الوردية | الحاله | وقت الدخول | وقت الخروج | التأخير | إجمالي العمل | موقع الدخول | موقع الخروج
     ...
     */
    // Completed
    private function generateFirstLastPdf(Collection $data, string $title, int $companyId, ?string $dateRange = null): void
    {
     ...
     */
    // Completed
    private function generateTimeRecordsPdf(Collection $data, string $title, int $companyId, ?User $employee = null, string $dateRange = ''): void
    {
     ...
     */
    // Completed
    private function generateDateRangePdf(Collection $data, string $title, int $companyId, ?User $employee = null, string $dateRange = ''): void
    {
     ...
     */
    // Completed
    private function generateTimesheetPdf(Collection $data, string $title, int $companyId, string $dateRange = ''): void
    {
        // Fetch Leave Types
     ...
     */
    // Completed
    private function generateLoanPdf(Collection $data, string $title, int $companyId, array $filters = []): void
    {
        $headers = [
            'الموظف',
            'الشهر والسنه',
    ...
    private function parseDurationToSeconds($duration): int
    {
        ...
        // Avoid dates
        if (str_contains((string)$duration, '-')) return 0;
