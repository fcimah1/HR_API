...
class UpdateJobRequest extends FormRequest
{
    ...
    {
        $id = $this->route('id');
        if (!\App\Models\Job::where('job_id', $id)->exists()) {
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'designation_id' => [
                'required',
                'integer',
                Rule::exists('ci_designations', 'designation_id')->where('company_id', $effectiveCompanyId)
            ],
            'job_type' => ['required', 'integer', Rule::enum(JobTypeEnum::class)],
            'job_vacancy' => 'integer|min:1',
            'gender' => ['required', 'integer', Rule::enum(GenderEnum::class)],
            'minimum_experience' => ['required', 'integer', Rule::enum(ExperienceLevel::class)],
            'date_of_closing' => 'date|after_or_equal:today|date_format:Y-m-d',
            'short_description' => 'string',
            'long_description' => 'string',
            'status' => ['required', 'integer', Rule::enum(JobStatusEnum::class)],
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'long_description.required' => 'الوصف المطول مطلوب',
            ...
            'minimum_experience.integer' => 'الخبرة المطلوبة يجب أن يكون رقماً',
